# this LM code is specific for the APNEWS dataset.
import numpy as np
...
class LM_apnews():
    ...
    def run_epoch(self, data, is_train=False, verbose=False):
        ...
        epoch_size = epoch_size = sum( [len(x["doc_lm"]) for x in data])
        ...
        steps = 0
        for doc in data:
            for step, (x, y) in enumerate(doc["doc_lm"]):
                feed_dict = {}
                ...
                if verbose and steps % 1000 == 0 and False:# (epoch_size // 10) == 10:
    ...
    def train_model(self, epochs=20, save_path_lm_model="language_model"):
        ...
        print("Sample: %s" % self.pretty_print(self.do_sample([self.reader.lm_word2id[word] for word in seed_for_sample],
                                                              max(5 * (len(seed_for_sample) + 1), length_sentence)), self.reader.lm_id2word))
        ...
        for epoch in range(epochs):
            ...
            print("epoch:", epoch, "train ppl:", train_ppl, "valid ppl:", valid_ppl, "test ppl:", test_ppl)



if __name__ == '__main__':
    # reader = Reader_PTB(datapath="../data/PTB", length_batch= 20, batch_size=20)
    # reader = Reader_APNEWS(datapath="../data/apnews/apnews.dat"  , length_batch=5, batch_size=5)

    reader_apnews = p.load(open("../saved/apnews.p", "rb"))
    print( "length batch", reader_apnews.length_batch)
    print( "batch size", reader_apnews.batch_size)
